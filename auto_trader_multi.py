...
def _get_conn():
    conn = sqlite3.connect(_get_db_path(), check_same_thread=False, timeout=30)
        ...
        try:
            ...
            if active:
                ...
                for o in active:
                    ...
                    if cur and avg:
                        ret = (cur - avg) / avg * 100
                        lines.append(f"  📌 {o['name']}  {ret:+.1f}%  (평단가 ₩{avg:,.0f}{split_tag} / 현재 ₩{cur:,.0f})")
                    else:
                        lines.append(f"  📌 {o['name']}  평단가 ₩{avg:,.0f}{split_tag}")
            if pending:
