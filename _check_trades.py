...
print("=== trade_orders ===")
rows = conn.execute("SELECT id,symbol,name,status,split_step,avg_price,qty,alert_date,trigger2,trigger3,base_price FROM trade_orders ORDER BY id DESC LIMIT 10").fetchall()
if rows:
    for r in rows:
        print(r)
else:
